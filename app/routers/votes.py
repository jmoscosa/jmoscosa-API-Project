...
# Here we will verify the post is trying to be liked actually exists
    ...
    if (vote.dir == 1):
        ...
        db.add(new_vote)
        db.commit()
        return{"message": "Your vote counts!"}
    else:
        if not found_vote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Unable to remove un-existing vote")
